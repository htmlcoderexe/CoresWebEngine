...
#}|#}  <div class="cal-week-header-wrapper">{#foreach|{%days%}|<span class="cal-week-header{:style:}"><a href="/calender/view/date/{:date:}"><span class="cal-day-label">{:title:}</span></a></span>#}</div>
    {#ifset|marker|{%marker%}<span id="marker" class="cal-week-marker"  style="left: calc({:xpos:}% + 3em); top: calc({:ypos:}em + 2em)">&nbsp;</span><script type="text/javascript">document.getElementById("marker").scrollIntoView();</script>|#}
    </div>
</div>
